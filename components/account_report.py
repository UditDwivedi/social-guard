...
def account_report(account_report_file):

    if os.path.exists(account_report_file):
        # Load the CSV file
        df = pd.read_csv(account_report_file)
        ...
        if True:  
            c = st.multiselect(
                "Choose Status", ["Red", "Green", "Yellow"], ["Red", "Green", "Yellow"]
            )
    ...
    terminate_file_path = "files/terminate.csv"
